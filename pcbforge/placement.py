...
def _render_brief(
    ...
    body = f"""# {project_name} placement brief

> Generated by PCBForge from `placement.yaml`. This is qualitative guidance,
> not placement geometry. The user owns every footprint position and all
> copper; the agent edits them only on explicit request during LAYOUT.
