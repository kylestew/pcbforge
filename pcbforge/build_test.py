...
def _report_metadata(text: str) -> Mapping[str, Any]:
    try:
        data = yaml.load(metadata_yaml(text), Loader=_UniqueLoader)
    except (ValueError, yaml.YAMLError) as exc:
        raise BuildTestError(f"invalid build-test report metadata: {exc}") from exc
    if not isinstance(data, dict):
        raise BuildTestError("build-test report metadata must be a mapping")
    return data
...
def _render_report(
    *,
    ...
    body = f"""# {project_name} build + test report
