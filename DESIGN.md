...
- **2026-08-20 — CIRCUIT review artifact is a generated KiCad schematic.**
  The schemdraw SVG is replaced by `review/circuit/circuit.kicad_sch`,
  generated by `pcbforge render-circuit` from an agent-authored
  `circuit_schematic.py` (`pcbforge.kicad_sch`). Capture stays in Atopile and
  `circuit.yaml`; the sheet is derived review support the user keeps open in
  eeschema while hand-laying the board. The 2026-07-24 Option D objection
  ("ERC and vibes") was about capture and does not apply: the gate now proves
  the sheet against the model pin for pin via `kicad-cli sch export netlist`,
  plus ERC and structural binding, which is stronger than the SVG's attribute
  coverage. Constraints: symbols embedded from the pinned KiCad 9 libraries,
  format `20250114`, no `.kicad_pro` under `review/` (no cross-probe in v1,
